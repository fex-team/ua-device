	...
	if (uaData.device.type == "mobile" || uaData.device.type == 'tablet') {
		...
		// hongmi有特殊判断build/hm2015011
			...
			if (tmp_match = ua.match(/(meitu|MediaPad)/i)) {
				...
				uaData.device['manufacturer'] = tmp_match[1];
				uaData.device['model'] = '';
			} 
			// 若匹配出的 match[2]没空格 会出现很多例如 mizi mizhi miha 但也会出现mi3 minote之类 特殊处理下
				...
				if (tmp_match = match[2].match(/(\d)/i)) {
					uaData.device['model'] = match[1] + '-' + tmp_match[1];
				}
			} else {
			...
			// mi-2w  mi-3w 等格式化为mi-2  mi-3
				...
				if(tmp_match = uaData.device['model'].match(/(mi|hm)[\s-_](note|pad)(\d?s?)/i)) {
					uaData.device['model'] = tmp_match[1] + '-' + tmp_match[2]+tmp_match[3];
				} else {
		...
		// handle meizu
			...
			if (match[3]){
				uaData.device['model'] = tmp_model + '-' + match[3];
			} else {
				uaData.device['model'] = tmp_model+'';
			}
