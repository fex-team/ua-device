...
module.exports = function (ua) {
    ...
    if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
        // 解决误判问题(经常误判为OPPO)
        // 至少一个空格,最后一个单词含有数字,则将最后一个单词作为 model 进行分离
        if (match = (uaData.device.model || '').match(/[0-9a-zA-Z]+$/i)) {
            uaData.device.manufacturer = trim(uaData.device.model.replace(match[0], ''));
            uaData.device.model = trim(match[0]);
        } else {
            ...
            if (match = ua.match(/(iPod|iPad|iPhone)/i)) {
                uaData.device.manufacturer = match[0];
